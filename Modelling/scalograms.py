...
#Function to generate and plot scalograms using CWT in one view using subplots
def plot_scalograms_side_by_side(dataframes, titles, col_names):
    fig, axes = plt.subplots(1, len(dataframes), figsize=(20, 4))  #adjust figsize if needed

    for i, (df, title, col_name) in enumerate(zip(dataframes, titles, col_names)):
        scales = np.arange(1, 64) #needs to be tuned
        coefficients, frequencies = pywt.cwt(df, scales, 'cmor') 
    ...
    plt.tight_layout(rect=[0, 0, 0.9, 1])  #adjusting the layout 
    plt.show()
