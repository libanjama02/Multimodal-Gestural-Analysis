...
# Define device MAC address and connect
...
# Global variable to store quaternion
current_rot = None

# Define callback function for quaternion data
def data_handler(ctx, data):
    ...
    if hasattr(values, 'w') and hasattr(values, 'x') and hasattr(values, 'y') and hasattr(values, 'z'):
        current_rot = Quaternion.from_q([values.w, values.x, values.y, values.z])
        print("Received quaternion:", current_rot)
    else:
        print("Parsed values do not contain expected attributes (w, x, y, z).")


# The Quaternion class and CubeAxes classes remain the same...
...
# Get quaternion signal and subscribe
signal_q = libmetawear.mbl_mw_sensor_fusion_get_data_signal(device.board, SensorFusionData.QUATERNION)
libmetawear.mbl_mw_datasignal_subscribe(signal_q, None, FnVoid_VoidP_DataP(data_handler))
...
# Function to continuously update the cube
def update_cube(ax):
    try:
        ...
        libmetawear.mbl_mw_datasignal_unsubscribe(signal_q)
        device.disconnect()
...
# Create and start a new thread for updating the cube
update_thread = threading.Thread(target=update_cube, args=(ax,))
update_thread.start()
