...
class CubeAxesTest(CubeAxes):
    def __init__(self, *args, **kwargs):
        super(CubeAxesTest, self).__init__(*args, **kwargs)
        self.current_rot = Quaternion.from_v_theta((1, 1, 0), np.pi / 6)  # Fixed rotation for testing
        self.draw_cube()

    def update(self):
        # Update with the fixed rotation (no real-time data yet)
        self.draw_cube()
        print("Updating visualization with fixed rotation.")
...
ax = CubeAxesTest(fig, rect=[0.1, 0.2, 0.8, 0.7])
fig.add_axes(ax)
...
# Real-time update loop with fixed rotations
try:
